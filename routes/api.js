...
var { Vokal, Base, tiktok, Searchnabi, Gempa } = require('../lib/index');
...
// Api Features :
        ...
        var randomTextNumber = random+randomlagi+'---------kuhong-api-storage'+'${creator}';
...
router.get('/tiktod', async (req, res, next) => {
	var apikeyInput = req.query.apikey,
         ...
         if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'wanz') return res.json(loghandler.invalidKey)
...
router.get('/flaming', async (req, res, next) => {
  ...
  try {
	 ...
	 var buffer = await (await fetch(`https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${json.result}&name=${creator}`)).json()
...
router.get('/yutub/video', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan parameter url"})
       ...
       fetch(encodeURI(`https://zenzapi.xyz/api/downloader/ytmp4?url=${url}&apikey=WanzBotz`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/yutub/audio', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "Masukan parameter url"})
       ...
       fetch(encodeURI(`https://zenzapi.xyz/api/downloader/ytmp3?url=${url}&apikey=WanzBotz`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
